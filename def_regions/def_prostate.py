  ...
  def __init__(self, pm, examination, ss, choices, site):
    ...
    if region == 'prostate':
		...
		# Hypofractionated prostate with vesicles (3 Gy x 20):
			...
			ptv_55 = ROI.ROIExpanded(ROIS.ptv.name, ROIS.ptv.type, COLORS.ptv_high, source = ROIS.prostate, margins = MARGINS.prostate_seed_expansion)
			ctv_55 = ROI.ROIExpanded(ROIS.ctv.name, ROIS.ctv.type, COLORS.ctv_high, source = ROIS.prostate)
			bladder_ptv = ROI.ROIAlgebra(ROIS.z_bladder.name, ROIS.z_bladder.type, COLORS.bladder, sourcesA = [ROIS.bladder], sourcesB = [ptv_55], operator='Subtraction', marginsB = MARGINS.uniform_3mm_expansion)
			rectum_ptv = ROI.ROIAlgebra(ROIS.z_rectum.name, ROIS.z_rectum.type, COLORS.rectum, sourcesA = [ROIS.rectum], sourcesB = [ptv_55], operator='Subtraction', marginsB = MARGINS.uniform_2mm_expansion)
			wall_ptv_55 = ROI.ROIWall(ROIS.z_ptv_wall.name, ROIS.z_ptv_wall.type, COLORS.wall, ptv_55, 1, 0)
			site.add_oars([bladder_ptv, rectum_ptv, wall_ptv_55])
			site.add_targets([ptv_55, ctv_55, ROIS.prostate])
    # Prostate bed:
    else:
