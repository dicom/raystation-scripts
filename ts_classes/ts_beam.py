...
class TSBeam(object):
  ...
  def segment_test(self):
    t = TEST.Test("Segment skal være definert.", True, self.mlc)
    if len(list(self.beam.Segments)) > 1:
      return t.succeed()
    else:
      return t.fail()
  ...
  # FIXME: For elektronikken er det bare alvorlig med stor feltåpning vekk fra gantry (ikke inn mot gantry), mens denne testen gir fail på stor åpning i begge retninger.
  # En mulig forbedring kan være å teste bare åpningen vekk fra gantry.
